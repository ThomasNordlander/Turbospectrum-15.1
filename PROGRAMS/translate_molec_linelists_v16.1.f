*
* This program translates various linelists into turbospectrum format.
* unified version by BPz 12-Jan-2017
*
...
*
     ...
     &          whichcah*1,whichoh*1,sys*3,molname*3,blabla*128,
     &          deltaN*1,deltaJ*1,e*1,mol*3,trans*2,branch2*7,
     &          identification*40,lab*3,levellow*2,levelup*2,
     &          translow*1,transup*1,sym*1,br*2,idwave*3,red*1,
     &          branch*13,
     &          idvald*14,levup*1,levlow*1,whichco*1,bla*15,
     &          bla1*15,bla2*15,bla3*15,bla4*15
     ...
     &        readCNsinglefile,readC2Brooke
      real tioisotopf(5),fact
      data tioisotopf/0.08, 0.073, 0.738, 0.055, 0.054/

* whichco='L' : Li et al. list 2015. ApJS, 216, 15
      whichco=' '
      isotopes='          '
      transition='      '
      iel=0
...
* we set jup=1 because we don't know what it is.
        ...
        isowant=30
        increasewave=.true.
      else if (elname.eq.'C1213B') then
* read Ram et al. (2014, ApJS 211, 5) 12C13C Swan linelist 
        elstring='0606.012013          '
        readC2Brooke=.true.
        isowant=1213
      else if (elname.eq.'C1212B') then
* read Brooke et al. (2013, JQSRT 124, 11) 12C12C Swan linelist 
...
* increasewave tells us that the line list is in increasing wavenumber order.
      ...
      else if (elname(1:4).eq.'26CO') then
        elstring='0608.012016          '
        readCO=.true.
        isowant=26
* for CO from  Li et al. list 2015. ApJS, 216, 15. Default is for Goorvitch CO.
        if (elname.eq.'26CO_L') then
          isowant=1
          whichco='L'
        endif
      else if (elname(1:4).eq.'27CO') then
        elstring='0608.012017          '
        readCO=.true.
        isowant=27
        if (elname.eq.'27CO_L') then
          isowant=2
          whichco='L'
        endif
      else if (elname(1:4).eq.'28CO') then
        elstring='0608.012018          '
        readCO=.true.
        isowant=28
        if (elname.eq.'28CO_L') then
          isowant=3
          whichco='L'
        endif
      else if (elname(1:4).eq.'36CO') then
        elstring='0608.013016          '
        readCO=.true.
        isowant=36
        if (elname.eq.'36CO_L') then
          isowant=4
          whichco='L'
        endif
      else if (elname(1:4).eq.'37CO') then
        elstring='0608.013017          '     
        readCO=.true.
        isowant=37
        if (elname.eq.'37CO_L') then
          isowant=5
          whichco='L'
        endif
      else if (elname(1:4).eq.'38CO') then
        elstring='0608.013018          '
        readCO=.true.
        isowant=38
        if (elname.eq.'38CO_L') then
          isowant=6
          whichco='L'
        endif
      else if (elname(1:4).eq.'46CO') then
        elstring='0608.014016          '
        readCO=.true.
        isowant=46
        if (elname.eq.'46CO_L') then
          isowant=7
          whichco='L'
        endif
      else if (elname(1:4).eq.'47CO') then
        elstring='0608.014017          '
        readCO=.true.
        isowant=47
        if (elname.eq.'47CO_L') then
          isowant=8
          whichco='L'
        endif
      else if (elname(1:4).eq.'48CO') then
        elstring='0608.014018          '
        readCO=.true.
        isowant=48
        if (elname.eq.'48CO_L') then
          isowant=9
          whichco='L'
        endif
...
c IR OH from HItran.
        ...
        readZrO=.true.
      else if (elname.eq.'93ZrO   ') then
        elstring='0840.000093          '
        readZrO=.true.
...
* if Kotlar data is available, we use it, ... unless
                wave=wavekotlar
                idwave='Kot'
              endif
* ... unless Ram et al. data exists
              if (waveram.gt.-1.0) then
...
* assume jup = jlow.
*************
          else if (readC2Brooke) then
* to date this is exclusively for 12C2 and 12C13C Swan band
* divide heteronuclear gf-values by 2 in order to use Irwin's partition function 
* made for 12C2 in turbospectrum. BPz 11/05-2015
            if (first) then
              read(10,'(a)') blabla
              i=1
            ...
            if (isowant.eq.1212) then
              read(10,1010,end=99) ivup,ivlow,jup,jlow,waveobs,wave,
     &                             chie,gf,branch
1010    format(i2,1x,i3,1x,i4,1x,i3,18x,2f13.0,29x,f12.0,15x,e14.6,a13)
              gf=gf*(2.*float(jlow)+1.)
* apply energy shift relative to X1sigma+ level (Kokkin et al. 2007, JCP 126, 084302
              chie=chie+709.95
              chie=chie*kayser2eV
              xjup=float(jup)
* if waveobs is not NaN, use it instead of calculated wavenumber
              if (waveobs.eq.waveobs) then 
                wave=waveobs
              endif
              write(identification,1011) '12C12C Brooke ',ivup,ivlow,
     &              jlow,branch
1011          format('''',a,i3,i3,i4,1x,a13,'''')
            else if (isowant.eq.1213) then
              read(10,1012,end=99) ivup,ivlow,jup,jlow,waveobs,wave,
     &                             chie,gf,branch
1012          format(i1,1x,i1,2(1x,i2),9x,f9.0,1x,f10.0,9x,f9.0,14x,
     &               f12.0,a)
              gf=gf*(2.*float(jlow)+1.)/2.
* apply energy shift relative to X1sigma+ level (Kokkin et al. 2007, JCP 126, 084302
              chie=chie+709.95
              chie=chie*kayser2eV
              xjup=float(jup)
* if waveobs is not 0, use it instead of calculated wavenumber
              if (waveobs.gt.0.) then 
                wave=waveobs
              endif
              write(identification,1013) '12C13C Ram ',ivup,ivlow,
     &              jlow,branch
1013          format('''',a,i3,i3,i4,1x,a10,'''')
            else
              print*,' you asked for isotope ',isowant
              stop 'This does not exist for Brooke line list'
            endif
*************
...
* corresponds to 12C12C, or 13C13C, whereas Quercis is for 12C13C).
          ...
          else if (readCO.and.whichco.ne.'L') then
* Older Goorvich CO data
...
*  we use here jlow in stead of jup, doesn't make a lot of difference 
          else if (readCO.and.whichco.eq.'L') then
* Li et al, 2015 CO line list. Format is HITRAN 160 characters record from 2004
326         read(10,325,end=99) isot,wave,A,chie,
     &                   bla1,bla2,bla3,bla4,bla,gup,glow
*
...
*
325         format(2x,i1,f12.6,10x,e10.3,10x,f10.4,12x,4a15,18x,
     &             a1,2f7.1)
cc325         format(i2,i1,f12.6,2e10.3,2f5.4,f10.4,f4.2,f8.6,4a15,6i1,
cc     &             6i2,a1,2f7.1)
            if (isot.ne.isowant) goto 326
            gf=1.499*gup*A/wave/wave
            chie=chie*kayser2eV
* this raddamp is not very good. It should be 1./lifetime of the level,
* which implies summing over all transition from that level.
            raddamp=A
            read(bla1,*) ivup
            read(bla2,*) ivlow
            xjup=(gup-1.)/2.
            xjlow=(glow-1.)/2.
            write(identification,1091) ivup,ivlow,xjup,xjlow
1091        format('''',i2,1x,i2,1x,f5.1,1x,f5.1,'''')
...
* E-B
            ...
            write(identification,1083) ivup,ivlow,branch2(1:4),jlow,
     &                                 system,lab(1:1)
          else if (readFeH) then
* added for FeH Plez, 12/11-2015
*FeH 02 00 008 + SR32   .1626E-04  13108.550   .086 F
            read(10,2001,end=99)  mol, ivup,ivlow,jlow,branch2,gf,
     &                         wave,chie,system
2001        format(a3,i3,i3,i4,3x,a4,e12.0,f11.0,f7.0,1x,a1)
            write(trans,'(a1,a1)') system, 'X'
            xjlow=float(jlow)+0.5
            if (branch2(2:2).eq.'P') then
              xjup=xjlow-1.
            else if (branch2(2:2).eq.'Q') then
              xjup=xjlow
            else if (branch2(2:2).eq.'R') then
              xjup=xjlow+1.
            else
              print*,branch2,'problem!'
              stop
            ...
            gup=2.*xjup+1.
            write(identification,1087) ivup,ivlow,branch2(1:4),xjlow,
     &                                 elname,trans
1087        format('''',i2,i3,1x,a4,1x,f4.1,1x,a7,1x,a2,'''')
          else
...
* maybe not jup, but jlow. The difference is in any case small.
     ...
     &                      levup,levlow,eqw,eqwerr,
     &                      adjustl(trim(identification))
             else
...
cc      print*,comment
      print*,elstring
      print 1234,elname,isotopes,transition,iel,ion
      print*,'number of lines selected before applying cut',nlinebef
      ...
      do while (.true.)
        ...
        if ((gflog+log10(lambda)-chie*5040./tempselect).ge.
     &        (weakratio+strongest)) then
          write(outfil,10) trim(blabla)
cc        else 
cc           print*,'weaker line ',gflog+log10(lambda)-chie*5040./
cc     &       tempselect,weakratio+strongest
cc           print*,trim(blabla)
        endif
      enddo
999   continue
      endif
      print*,'number of lines written after applying cut  ',nline
      close(30)
      stop
...
20    format(f10.3,x,f6.3,x,f6.3,x,f5.2,x,f6.1,x,e8.2,1x,
     & '''',a,'''',1x,'''',a,'''',1x,f4.1,1x,f4.1,1x,a)
21    format(f10.3,x,f6.3,x,f6.2,x,f5.2,x,f6.1,x,e8.2,1x,
     & '''',a,'''',1x,'''',a,'''',1x,f4.1,1x,f4.1,1x,a)

cc20    format(f10.3,x,f6.3,x,f6.3,x,f5.2,x,f6.1,x,e8.2,1x,a)
cc21    format(f10.3,x,f6.3,x,f6.2,x,f5.2,x,f6.1,x,e8.2,1x,a)
...
1234  format(a6,1x,a10,1x,a6,' iel ',i4,' ion ',i2)

...
*
* This version is v2. Can handle 'EXTRACT STELLAR' format from VALD3
* BPz 16/01-2014
...
*
      ...
      print*,' The format for molecular lines is compatible'
      print*,' with B. Plez''''s Turbospectrum v13.1 and later.'
      ...
      print*
      print*,' stellar or all format?'
      read(5,'(a)') valdformat
      print *, 'using ', valdformat
      ...
      ! FIXME: ask if isotopic scaling should be undone
      ! The VALD3 documentation should have a list of isotopic ratios
      irec=0
      nread=0
      nlines=0
      nskip=0
...
* FORTRAN does not recognize comma as a delimiter, confirm fixed read format:
          if(     string(24:24).eq.','
     &       .and.string(33:33).eq.','.and.string(42:42).eq.','
     &       .and.string(48:48).eq.','.and.string(57:57).eq.','
     &       .and.string(63:63).eq.','.and.string(71:71).eq.','
     &       .and.string(79:79).eq.','.and.string(87:87).eq.','
     &       .and.string(94:94).eq.','.and.string(101:101).eq.','
     &        ) then
     
* BPz: comma at position 105 does not exist with extended VdW information.
cc     &       .and.string(105:105).eq.',') then
* OK, this is the expected format, read the rest of the lines' data
            
            nread=nread+1
            length=len_trim(string)
            if(length.lt.106) then
* BPz: try to accomodate "extract stellar" format
cc            if(length.ne.106) then
* BPz : length is 106 with extended VdW information
cc            if(length.ne.105) then
              print *,string,' : length=',length
              stop 'Wrong length of the first record'
            endif
            string1=string(1:106)
* BPz: try to accomodate "extract stellar" format
cc            string1=string(1:length)
            read(string1,*) specname
cc            print*,specname
            read(10,'(a)') string  ! reading line 2: lower configuration
            irec=irec+1
            length=len_trim(string)
            if(length.eq.0) then
              print *,'No lower level data, file record number',irec
              print *,string1
            else if(length.ne.92) then
              print *,string,' : length=',length
              stop 'Wrong length of the second record'
            endif
            string2=string(1:length)
            read(10,'(a)') string  ! reading line 3: upper configuration
            irec=irec+1
            length=len_trim(string)
            if(length.eq.0) then
              ...
              print *,string,' : length=',length
              stop 'Wrong length of the third record'
            endif
            string3=string(1:length)
            read(10,'(a)') string4  ! reading line 4: references
            irec=irec+1
* BPz: try to accomodate "extract stellar" format (full2 case)
            full1=string4(145:158)
!            full2=string4(36:49)
            full2=string4(29:42) ! updated position for bibliographic species info
1           continue
            if (valdformat.eq.'all') then
              full=full1
            else if (valdformat.eq.'stellar') then
              full=full2
            else
              stop 'valdformat undefined'
            endif
            call identify(full,lele,species,ionic,elem1,elem2,specname,
     &                    problem)
            if (problem) then
cc* if 'all' format does not work, we return with full set to empty string
cc              if (valdformat.eq.'all') then
cc                valdformat='stellar'
cc                goto 1
cc              else if (valdformat.eq.'stellar') then
...
*             print *,'input file record number',irec
            ...
            read(string1,'(33x,f8.4,7x,f8.4)') chil,chiu
            if(chil.ge.15.) then
...
*OK, extract data and write
 ...
 1121     format('''',a11,9x,'''',i5,i10,/,'''',a7,'''')
        ...
        do i=1,nu(ii)
          gam6=0.0
          read(s1(n),1010) w,gflog,chil,chiu,rjupper,gamrad,gam6
*BPz: format changed to allow reading BPM broadening "xxx.yyy", as well as gam6 "-x.yyy"
 1010     format(9x,f14.4,x,f8.3,x,f8.4,7x,f8.4,x,f5.1,22x,f6.3,
     &           8x,f8.3)
cc     &           8x,f6.3)
          if(.not.molecule) then
* Atomic species
...
*
*
      subroutine identify(fullspec,lele,species,ion,lek,lel,specname,
     &                    problem)
* identify atomic or molecular lines, ionization, and isotopes
*
      implicit none
      integer      length,itype,iso(2),istart1,istart2,iiso2,i,iel(2)
      integer      iichar(14),ion,natom,iatom(2)
      character*14 fullspec,fff,full
      character*11 species
      character*6  isostring
      character*2  lele(92),lek,lel
      character*1  cchar(14)
      character specname*100,haha*100
      logical problem
      equivalence (fff,cchar)
*
      problem=.false.
1     fff=fullspec
      iel(1)=0
...
* Cases with isotopes:
        if(cchar(3).eq.')') then
          read(cchar(2),'(i1)') iso(1)
          istart1=4
        else if(cchar(4).eq.')') then
          read(fullspec(2:3),'(i2)') iso(1)
          istart1=5
        else if(cchar(5).eq.')') then
...
* OK, a second isotope found, i.e. a molecule
        if(cchar(iiso2+1).eq.')') then
          read(cchar(iiso2),'(i1)') iso(2)
          istart2=iiso2+2
        else if(cchar(iiso2+2).eq.')') then
          read(fullspec(iiso2:iiso2+1),'(i2)') iso(2)
          istart2=iiso2+3
        else if(cchar(iiso2+3).eq.')') then
...
* OK, we now have any isotope information, find 1 or 2 elements:
      ...
      lel='  '
      do i=1,14
        call case(.false.,cchar(i),iichar(i))
        if(iichar(i).eq.1) then
          natom=natom+1
          iatom(natom)=i
        endif
      enddo
cc      print*,'check',specname,natom, iatom,iichar(iatom(1)),
cc     &         iichar(iatom(2))
      lek(1:1)=cchar(iatom(1))
      if(iichar(iatom(1)+1).ne.-1) then
* 1-character atom
        lek(2:2)=' '
cc        print*,'1','lek',lek,'name',specname
cc        if (lek(1:1).ne.specname(1:1)) then
* need to signal problem
cc          problem=.true.
cc          return
cc        endif
        call findatom(lek,lele,iel(1))
        if(cchar(iatom(1)+1).eq.'+' .and. iichar(iatom(1)+2).eq.3) then
          read(cchar(iatom(1)+2),'(i1)') ion
          ion=ion+1
        else if(cchar(iatom(1)+1).eq.'+') then
          ion=2
        else if(cchar(iatom(1)+1).eq.' ') then
          ion=1
        else if(iichar(iatom(1)+1).eq.1) then
          continue
        else if(cchar(iatom(1)+1).eq.'(') then
          continue
        else
...
* 2-character atom
        lek(2:2)=cchar(iatom(1)+1)
cc        print*,'2','lek',lek,'name',specname
cc        if (lek(1:2).ne.specname(1:2)) then
cc* need to signal problem
cc          fullspec='               '
cc          problem=.true.
cc          return
cc        endif
        call findatom(lek,lele,iel(1))
        if(cchar(iatom(1)+2).eq.'+' .and. iichar(iatom(1)+3).eq.3) then
          read(cchar(iatom(1)+3),'(i1)') ion
          ion=ion+1
        else if(cchar(iatom(1)+2).eq.'+') then
          ion=2
        else if(cchar(iatom(1)+2).eq.' ') then
          ion=1
        else if(iichar(iatom(1)+2).eq.1) then
          continue
        else if(cchar(iatom(1)+2).eq.'(') then
          continue
        else
...
*     print *,'FULL,lek,ion,iel(1-2)',fullspec,lek,ion,iel
      ...
      if(natom.eq.2) then
cc        print*,natom,' natom'
cc        print*,specname
cc        print*,iatom(1),iatom(2)
        lel(1:1)=cchar(iatom(2))
        if(iichar(iatom(2)+1).ne.-1) then
* 1-character atom
          lel(2:2)=' '
cc        print*,'3','lel',lel,'name',specname
cc          if (lel(1:1).ne.specname(3:3)) then
cc* need to signal problem
cc            problem=.true.
cc            return
cc          endif
          call findatom(lel,lele,iel(2))
          if(cchar(iatom(2)+1).eq.'+'.and.iichar(iatom(2)+2).eq.3) then
            read(cchar(iatom(2)+2),'(i1)') ion
            ion=ion+1
          else if(cchar(iatom(2)+1).eq.'+') then
            ion=2
          else if(cchar(iatom(2)+1).eq.' ') then
            ion=1
          else
...
* 2-character atom
          lel(2:2)=cchar(iatom(2)+1)
cc        print*,'4','lel',lel,'name',specname
cc          if (lel(1:2).ne.specname(3:4)) then
cc* need to signal problem
cc            problem=.true.
cc            return
cc          endif
          ...
          if(iatom(2).lt.13) then
            if(cchar(iatom(2)+2).eq.'+'.and.iichar(iatom(2)+3).eq.3)then
              read(cchar(iatom(2)+3),'(i1)') ion
              ion=ion+1
            else if(cchar(iatom(2)+2).eq.'+') then
              ion=2
            else if(cchar(iatom(2)+2).eq.' ') then
              ion=1
            else
...
cc      do i=1,len(specname)
cc        if (specname(i:i).eq.' ') then
cc          haha=specname(1:i-1)
cc          exit
cc        endif
cc      enddo
* I remove tyhis as it will cause problem when specname='C2', and lek+lel='CC'
* BPz 1/04-2015
cc      if (trim(lek)//trim(lel).ne.specname(1:index(specname,' '))) then
cc        print*,'specname ', trim(specname),' different from ',
cc     &          trim(lek),trim(lel)
cc        print*, 'identify problem !'
cc        problem=.true.
cc        return
cc      endif
...
*
*
      subroutine case(change,c,iichar)
* changes upper to lower case if  change. and iichar=2
* changes lower to upper case if  change. and iichar=-2
* or
* determines whether "c" is upper case if .not.change.: output iichar=1
* determines whether "c" is lower case if .not.change.: output iichar=-1
* determines whether "c" is a number if .not.change.: output iichar=3
* or
* determines that "c" is not a letter/number if .not.change.: output iichar=0
* 
* c and iichar can be either input or output or both
      integer iichar
      character*1 c
      logical change
*
